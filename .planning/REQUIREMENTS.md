...
| REQ-SIZE-02 | 10-01, 10-08 | ⚠️ **half discharged** — the shared exclude list is real and locally tested (36 guards, mutation-checked). The **CI half has never executed**: 10-08's wiring is unpushed, and the workflows on `origin/main` have failed on all three platforms since ≥2026-07-22 at dependency install (`tinyml-tensorlab/tinyml-modelzoo` is not a Python project), before any test runs. The requirement's own words — "a build that loses the exclusions **fails CI**" — remain undemonstrated. The branch refs were fixed 2026-08-02 (`2c3fe31`) — both workflows pointed at branches that could not satisfy the install step — but nothing has run yet, since the fix is unpushed. See `10-DOC-AUDIT.md` H-2 |
